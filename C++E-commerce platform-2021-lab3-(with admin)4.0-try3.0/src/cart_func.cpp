...
int cart_func::generate_order()
{
    ...
    {
        if (temp->get_cart_owner() == cart_owner && temp->cart_status == 0) //找到这个消费者的且未生成订单
        {
            //先看下这个商品是否还真的存在
            if (server::check_goods_exist(temp->get_cart_goods_type(), temp->set_cart_goods_name(), temp->set_cart_goods_owner()) == -1)
            {
                cout << "The goods named " << temp->get_cart_goods_name() << " has been withdrawn. We will delete it from your cart." << endl;
                for (vector<cart *>::iterator it = v_cart.begin(); it != v_cart.end();)
                {
                    if (*it == temp)
                        it = v_cart.erase(it);
            ...
            else
            {
                ...
                {
                    << ". We change cout << "The quantity you want is over the surplus of " << temp->get_cart_want()->get_goods_name() it to the max you can buy automatically: " << temp->get_cart_want()->get_goods_surplus() << endl;
