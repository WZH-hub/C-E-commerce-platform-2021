...
void in_out::read_in_cart(vector<cart *> &p)
{
    ifstream fp("cart.txt"); //打开文件
    if (!fp.is_open())
    {
        cout << "error,can't open cart.txt" << endl;
        return;
    }
    string temp;
    while (getline(fp, temp))
    {
        ...
        getline(fp, cart_goods_name);
        cout << cart_goods_name << endl;
        cout << cart_goods_owner << endl;
        cout << cart_goods_type << endl;
        int flag = 0;
        for (goods *tmp : v_goods)
        {
            cout << "~~~~~~~~~~~~~~~~~~~~~" << endl;
            if (tmp->get_goods_name() == cart_goods_name && tmp->get_goods_owner() == cart_goods_owner && tmp->get_goods_type() == cart_goods_type)
            {
                flag = 1;
                cart_want = tmp;
                cout << cart_want->get_goods_name() << endl;
                cout << cart_want->get_goods_owner() << endl;
                cout << cart_want->get_goods_type() << endl;
            }
        }
        if (flag == 1)
        {
            cart *p_temp = new cart(cart_status, cart_owner, cart_want, cart_quantity, cart_goods_type, cart_goods_name, cart_goods_owner);
            p.push_back(p_temp);
        }
        else
        {
            cart *p_temp = new cart(cart_status, cart_owner, NULL, cart_quantity, cart_goods_type, cart_goods_name, cart_goods_owner);
            p.push_back(p_temp);
        }
    }
    fp.close();
    cout << "Read in cart.txt successfully." << endl;
}
...
void in_out::read_out_cart(const vector<cart *> p)
{
    ofstream fp("cart.txt");
    if (!fp.is_open()) //检测是否成功打开
    {
        cout << "error,can't open cart.txt" << endl;
        return;
    ...
    {
        fp << temp->cart_status << " ";
        fp << temp->get_cart_owner() << " ";
        fp << temp->get_cart_quantity() << " ";
        fp << temp->cart_goods_owner << " ";
        fp << temp->cart_goods_type << "\n";
        fp << temp->cart_goods_name << endl;
    }
    fp.close();
    cout << "Read out cart.txt successfully." << endl;
}
...
void in_out::delete_cart(vector<cart *> &p)
{
    for (vector<cart *>::iterator it = p.begin(); it != p.end(); it++)
    {
        if (NULL != *it)
        {
            delete *it;
